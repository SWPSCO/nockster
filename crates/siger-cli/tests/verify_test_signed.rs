...
use std::fs;
use tx_types::transaction_types::{SpendBody, Hash, SchnorrSignature, Chal, Sig, T8};
    ...
    for (name, spend) in v1.spends.map.tap() {
        ...
        if let SpendBody::V1(sb) = &spend.body {
            ...
            for (pkh, sig_val) in sb.witness.pkh.map.tap() {
                ...
                println!("  PK.x: {:016x}_{:016x}_{:016x}_{:016x}_{:016x}_{:016x}",
                    pk.x.values[0], pk.x.values[1], pk.x.values[2],
                    pk.x.values[3], pk.x.values[4], pk.x.values[5]);
                println!("  PK.y: {:016x}_{:016x}_{:016x}_{:016x}_{:016x}_{:016x}",
                    pk.y.values[0], pk.y.values[1], pk.y.values[2],
                    pk.y.values[3], pk.y.values[4], pk.y.values[5]);

                println!("\n  Attempting verification with sig_hash...");

                // Verify signature
                let verified = schnorr_verify_digest(
                    pk.clone(),
                    sig_hash.clone(),
                    sig_val.sig.clone(),
                );
                ...
                if verified {
                    ...
                    let verified_kg = schnorr_verify_digest(
                        pk.clone(),
                        known_good,
                        sig_val.sig.clone(),
                    );
