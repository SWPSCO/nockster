...
fn hash_note_data(map: Noun, arena: &Arena) -> Result<[u64; 5], JsValue> {
    ...
    let (node, left, right) = decompose_map(map, arena).ok_or_else(|| js_err("malformed note-data"))?;
    let (key, value) = decompose_pair(node, arena).ok_or_else(|| js_err("malformed note-data node"))?;

    let key_digest = hash_noun_varlen(key, arena).map_err(|e| js_err(format!("{e:?}")))?;
    let value_digest = hashable_noun_digest(value, arena)?;
    let node_digest = hash_ten_cell(key_digest, value_digest).map_err(|e| js_err(format!("{e:?}")))?;

    let left_digest = hash_note_data(left, arena)?;
    let right_digest = hash_note_data(right, arena)?;
    let children_digest = hash_ten_cell(left_digest, right_digest).map_err(|e| js_err(format!("{e:?}")))?;
...
fn hash_lock_primitive_hashable(lp: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    ...
    if arena.atom_eq_bytes(header_id, b"hax") {
        let tag_digest = hash_noun_varlen(header, arena).map_err(|e| js_err(format!("{e:?}")))?;
        return Ok(hash_ten_cell(tag_digest, ctx.fake_digest).map_err(|e| js_err(format!("{e:?}")))?);
    }

    if arena.atom_eq_bytes(header_id, b"brn") {
        let tag_digest = hash_noun_varlen(header, arena).map_err(|e| js_err(format!("{e:?}")))?;
        return Ok(hash_ten_cell(tag_digest, ctx.null_digest).map_err(|e| js_err(format!("{e:?}")))?);
    }

    Err(js_err("unsupported lock primitive"))
}

fn hash_lock_primitives_list(prims: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    if prims == arena.atom0() {
        return Ok(ctx.null_digest);
    }
    let (head, tail) = uncons(prims, arena).ok_or_else(|| js_err("malformed lock primitive list"))?;
...
fn hash_merkle_proof_hashable(merk: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
...
fn hash_lock_merkle_proof_hashable(lmp: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    let (spend_condition, _axis, merk_proof) =
        tuple3(lmp, arena).ok_or_else(|| js_err("malformed lock merkle proof"))?;
    ...
    let inner = hash_ten_cell(axis_digest, merk_digest).map_err(|e| js_err(format!("{e:?}")))?;
...
fn hash_hax_map_hashable(map: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    ...
    let (node, left, right) = decompose_map(map, arena).ok_or_else(|| js_err("malformed hax map"))?;
    let (key, value) = decompose_pair(node, arena).ok_or_else(|| js_err("malformed hax map node"))?;
...
fn hash_pkh_signature_map_hashable(map: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    if map == arena.atom0() {
        return Ok(ctx.null_digest);
    }
    let (node, left, right) = decompose_map(map, arena).ok_or_else(|| js_err("malformed pkh map"))?;
    let (key, value) = decompose_pair(node, arena).ok_or_else(|| js_err("malformed pkh map node"))?;
...
fn hash_seed_regular_hashable(seed: Noun, arena: &Arena) -> Result<[u64; 5], JsValue> {
    ...
    let lock_root = parse_hash(lock_root_noun, arena).ok_or_else(|| js_err("malformed seed lock-root"))?;
    ...
    let parent_hash = parse_hash(parent_hash_noun, arena).ok_or_else(|| js_err("malformed seed parent-hash"))?;
...
fn hash_spend_v1_hashable(spend: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    ...
    let (witness, seeds, fee) = tuple3(body_noun, arena).ok_or_else(|| js_err("malformed spend body"))?;
...
fn hash_spends_hashable(spends: Noun, arena: &Arena, ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    ...
    let (node, left, right) = decompose_map(spends, arena).ok_or_else(|| js_err("malformed spends"))?;
    let (name_noun, spend_noun) = decompose_pair(node, arena).ok_or_else(|| js_err("malformed spends node"))?;

    let name_digest = hash_nname_hashable(name_noun, arena)?;
    let spend_digest = hash_spend_v1_hashable(spend_noun, arena, ctx)?;
    let node_digest = hash_ten_cell(name_digest, spend_digest).map_err(|e| js_err(format!("{e:?}")))?;
...
fn calculate_witness_fee(witness: Noun, arena: &Arena) -> u64 {
    let leaves = count_leaves(witness, arena);
    let word_fee = BASE_FEE.saturating_mul(leaves);
    word_fee.max(MIN_FEE)
}

fn calculate_seed_fee(note_data: Noun, map_entries: u64, arena: &Arena) -> u64 {
    let raw_leaves = count_leaves(note_data, arena);
    let leaves = raw_leaves.saturating_sub(map_entries);
    let word_fee = BASE_FEE.saturating_mul(leaves);
    word_fee.max(MIN_FEE)
}
...
fn build_pkh_lock_multisig(
    ...
    for digest in pkhs {
        ...
        set = canonical_zset_put(arena, set, hash_noun).map_err(|e| js_err(format!("zset: {e:?}")))?;
    }
...
fn compute_first_name_from_lock_hash(lock_hash: [u64; 5], ctx: &TxIdCtx) -> Result<[u64; 5], JsValue> {
    Ok(hash_ten_cell(ctx.null_digest, lock_hash).map_err(|e| js_err(format!("{e:?}")))?)
}

fn build_witness_unsigned(arena: &mut Arena, lock: Noun, lock_hash: [u64; 5]) -> Noun {
    ...
    let lmp = build_tuple(arena, &[lock, axis, merkle_proof]);
...
fn witness_with_placeholder(witness_unsigned: Noun, placeholder_pkh_map: Noun, arena: &mut Arena) -> Result<Noun, JsValue> {
...
fn build_note_spec(
    ...
    let version_digest = hash_noun_varlen(version_noun, arena).map_err(|e| js_err(format!("{e:?}")))?;
    let origin_digest = hash_noun_varlen(origin_noun, arena).map_err(|e| js_err(format!("{e:?}")))?;
    let assets_digest = hash_noun_varlen(assets_noun, arena).map_err(|e| js_err(format!("{e:?}")))?;
    ...
    let witness_unsigned = build_witness_unsigned(arena, lock, lock_hash);
    let placeholder_map = build_placeholder_pkh_map(arena, source_pkh)?;
    let simulated_witness = witness_with_placeholder(witness_unsigned, placeholder_map, arena)?;
    let witness_fee = calculate_witness_fee(simulated_witness, arena);

    let capacity = note
        .assets
        .saturating_sub(witness_fee)
        .saturating_sub(refund_seed_fee);
...
fn build_output_seed_spec(
    ...
    let lock_hash = hash_lock_primitives_list(lock, arena, ctx)?;
    let (note_data, note_data_entries) = build_note_data_with_lock(arena, lock)?;
    let fee = calculate_seed_fee(note_data, note_data_entries, arena);
...
pub fn compose_tx_v1_recipient_address(recipient: JsValue) -> Result<String, JsValue> {
    let recipient: RecipientInput =
        serde_wasm_bindgen::from_value(recipient).map_err(|e| js_err(format!("bad recipient: {e}")))?;
...
fn split_seeds_to_capacity(mut seeds: Vec<SeedSpec>, capacity: u64) -> Result<Vec<SeedSpec>, JsValue> {
    let mut processed = Vec::new();

    for seed in seeds.drain(..) {
        let total_cost = seed.gift.saturating_add(seed.fee);
        ...
        while remaining > 0 {
            let chunk_amount = if remaining.saturating_add(seed.fee) <= capacity {
                remaining
            } else {
                capacity.saturating_sub(seed.fee)
            };
            if chunk_amount == 0 {
                return Err(js_err("cannot split seed to capacity (seed fee exceeds note capacity)"));
            }
...
fn consolidate_seeds(seeds: Vec<SeedSpec>, arena: &Arena) -> Vec<SeedSpec> {
    ...
    'outer: for seed in seeds {
        for existing in out.iter_mut() {
            if existing.lock_root == seed.lock_root && nouns_equal(existing.note_data, seed.note_data, arena) {
...
fn build_seed_noun(
    ...
    build_tuple(arena, &[output_source, lock_root, seed.note_data, gift, parent_hash])
}

#[derive(Debug)]
struct SpendBuildResult {
    spend: Noun,
    leftover: Vec<SeedSpec>,
...
fn construct_spend(
...
) -> Result<SpendBuildResult, JsValue> {
    let total_spendable = note.assets.checked_sub(note.witness_fee).ok_or_else(|| js_err("note cannot cover witness fee"))?;
    let mut spendable_for_seeds = total_spendable
        .checked_sub(refund_seed_fee)
        .ok_or_else(|| js_err("note cannot cover witness + refund seed fee"))?;

    let mut accepted: Vec<SeedSpec> = Vec::new();
    let mut leftover: Vec<SeedSpec> = Vec::new();
    let mut fees = note.witness_fee;

    for seed in seeds {
        let total_cost = seed.gift.saturating_add(seed.fee);
        if spendable_for_seeds >= total_cost {
            spendable_for_seeds -= total_cost;
            fees = fees.saturating_add(seed.fee);
            accepted.push(seed);
        } else {
            leftover.push(seed);
        }
    ...
    let remaining = spendable_for_seeds.saturating_add(refund_seed_fee);
    let mut refund_gift = 0u64;
    if remaining > refund_seed_fee {
        refund_gift = remaining - refund_seed_fee;
        fees = fees.saturating_add(refund_seed_fee);
        // refund seed fee already accounted for; seed itself added below as a normal seed.
        ...
        fees = fees.saturating_add(remaining);
    }

    // Sanity tally: assets == fees + gifts
    let mut tally = note.assets.checked_sub(fees).ok_or_else(|| js_err("fee tally underflow"))?;
    ...
    let fee_noun = arena.alloc_atom_u64(fees);
    let spend_body = build_tuple(arena, &[note.witness_unsigned, seed_set, fee_noun]);
...
fn compose_tx_v1_unsigned_inner(input: ComposeTxV1Input) -> Result<ComposedTransactionV1, JsValue> {
    ...
    let mut arena = Arena::new();
    let null_digest = hash_noun_varlen(arena.atom0(), &arena).map_err(|e| js_err(format!("{e:?}")))?;
    let fake_atom = arena.alloc_atom_bytes(b"fake");
    let fake_digest = hash_noun_varlen(fake_atom, &arena).map_err(|e| js_err(format!("{e:?}")))?;
    let version_atom = arena.alloc_atom_u64(1);
    let version_digest = hash_noun_varlen(version_atom, &arena).map_err(|e| js_err(format!("{e:?}")))?;
    let ctx = TxIdCtx {
        null_digest,
        fake_digest,
        version_digest,
    ...
    // Precompute refund seed fee using the standard output lock for the source pkh.
    ...
    let refund_seed_fee = calculate_seed_fee(refund_note_data, refund_note_data_entries, &arena);
    ...
    // Pack seeds into notes and build spends map.
    ...
    for note in &notes {
        ...
        remaining_seeds = spend_res.leftover;

        spends_map = canonical_zmap_put(&mut arena, spends_map, note.name_noun, spend_res.spend)
            .map_err(|e| js_err(format!("zmap: {e:?}")))?;

        // Build per-input witness-data and display.inputs metadata to satisfy wallet tooling.
        let (_spend_version, spend_body) = tuple2(spend_res.spend, &arena)
            .ok_or_else(|| js_err("malformed spend"))?;
        let (witness, _seeds, _fee) = tuple3(spend_body, &arena)
            .ok_or_else(|| js_err("malformed spend body"))?;

        witness_data_map = canonical_zmap_put(&mut arena, witness_data_map, note.name_noun, witness)
            .map_err(|e| js_err(format!("zmap: {e:?}")))?;
        inputs_display_map = canonical_zmap_put(&mut arena, inputs_display_map, note.name_noun, note.lock)
            .map_err(|e| js_err(format!("zmap: {e:?}")))?;

        total_fees = total_fees.saturating_add(spend_res.fee);

        summary_inputs.push(serde_json::json!({
            "name_first": hash_to_b58(note.name_first),
            "name_last": hash_to_b58(note.name_last),
            "assets": note.assets,
            "witness_fee": note.witness_fee,
        ...
        summary_spends.push(serde_json::json!({
            ...
            "fee": spend_res.fee,
            "refund": spend_res.refund_gift,
            "seeds": spend_res.accepted.iter().map(|s| serde_json::json!({
    ...
    if !remaining_seeds.is_empty() {
        return Err(js_err("insufficient funds to cover all outputs (after fees)"));
    }
    ...
    let wallet_tx = build_tuple(&mut arena, &[v1_tag, wallet_name, spends_map, display, witness_data]);
    ...
    let summary = serde_json::json!({
        ...
        "refund_seed_fee": refund_seed_fee,
...
mod tests {
    ...
    fn wallet_tx_v1_populates_witness_data_and_inputs_display() {
        ...
        let (wd_tag, wd_map) =
            tuple2(witness_data, &decode_arena).expect("witness-data tuple2");
